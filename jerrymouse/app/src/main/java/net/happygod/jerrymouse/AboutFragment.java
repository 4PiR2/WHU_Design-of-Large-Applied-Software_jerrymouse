...
public class AboutFragment extends Fragment
{
	...
	/****************
	 *
	 * 发起添加群流程。群号：524873785 的 key 为： 7GiBHIJ9P3KggadECzB7xBkwFeh9wKom
	 * 调用 joinQQGroup(7GiBHIJ9P3KggadECzB7xBkwFeh9wKom) 即可发起手机QQ客户端申请加群 524873785
	 *
	 * @param key 由官网生成的key
	 * @return 返回true表示呼起手机QQ成功，返回false表示呼起失败
	 ******************/
	public boolean joinQQGroup(String key) {
		Intent intent = new Intent();
		intent.setData(Uri.parse("mqqopensdkapi://bizAgent/qm/qr?url=http%3A%2F%2Fqm.qq.com%2Fcgi-bin%2Fqm%2Fqr%3Ffrom%3Dapp%26p%3Dandroid%26k%3D" + key));
		// 此Flag可根据具体产品需要自定义，如设置，则在加群界面按返回，返回手机QQ主界面，不设置，按返回会返回到呼起产品界面    //intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
		try {
			startActivity(intent);
			return true;
		} catch (Exception e) {
			// 未安装手机QQ或安装的版本不支持
			Const.toast("未安装手机QQ或安装的版本不支持", Toast.LENGTH_SHORT);
			return false;
		}
	}
